...
def set_L_value(val):
    ...
    L_entry.insert(0, str(val))
    L_entry.configure(state="disabled")
...
def on_channel_type_change(choice):
   ...
   # Handle DS enable/disable
    ...
    # if "3GPP" in selected_channel_type:
    # # set default subtype tap count
    #     subtype = selected_channel_subtype
    #     if "TDL-A" in subtype:
    #         set_L_value(23)
    #     elif "TDL-B" in subtype:
    #         set_L_value(25)
    #     elif "CDL" in subtype:
    #         set_L_value(5)
    if selected_channel_subtype in MODEL_TAPS:
        set_L_value(MODEL_TAPS[selected_channel_subtype])
    update_plots()
...
#     # set fixed taps for 3GPP
#     if "TDL-A" in choice:
#         set_L_value(23)
#     elif "TDL-B" in choice:
#         set_L_value(25)
#     elif "TDL-C" in choice:
#         set_L_value(23)   # change if needed
#     elif "CDL" in choice:
#         set_L_value(5)    # your placeholder
#     current_batch = 0
...
def on_channel_subtype_change(choice):
    ...
    # set correct taps
    if choice in MODEL_TAPS:
        set_L_value(MODEL_TAPS[choice])
...
def generate_ofdm_channel(Nsc, Nsym, fc, fd, B, L_user, DS, ch_type, ch_subtype):
    ...
    #         powers_db = np.array([0, -3, -6, -9, -12])
        ...
        # ---------- TDL ----------
        if "TDL-A" in ch_subtype:
            delays_norm = np.array([...])   # keep yours
            powers_db = np.array([...])
        ...
        elif "TDL-C" in ch_subtype:
            # simple extension (you can refine later)
            delays_norm = np.linspace(0, 6, 24)
            powers_db = -np.linspace(0, 25, 24)

        # ---------- CDL ----------
        elif "CDL-A" in ch_subtype:
            delays_norm = np.linspace(0, 5, 23)
            powers_db = -np.linspace(0, 22, 23)

        elif "CDL-B" in ch_subtype:
            delays_norm = np.linspace(0, 5.5, 23)
            powers_db = -np.linspace(0, 23, 23)

        elif "CDL-C" in ch_subtype:
            delays_norm = np.linspace(0, 6, 24)
            powers_db = -np.linspace(0, 25, 24)

        elif "CDL-D" in ch_subtype:
            delays_norm = np.linspace(0, 4, 14)
            powers_db = -np.linspace(0, 18, 14)

        elif "CDL-E" in ch_subtype:
            delays_norm = np.linspace(0, 4.5, 15)
            powers_db = -np.linspace(0, 20, 15)
